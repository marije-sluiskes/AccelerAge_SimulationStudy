...
GetB_AFTmrl_Gompertz <- function(df_train, df_test, M, lt){
  ...
  return(list(b_pred = aft_b, mrl_pred = pred_mrl, rmse = rmse, betas_est = betas_est_aft,
...
GetB_GrimAge <- function(df_train, df_test){
  ...
  linpred_train <- unname(predict(fit_PH, newdata = df_train, type = "lp", reference = "zero"))
  linpred_test <- unname(predict(fit_PH, newdata = df_test, type = "lp", reference = "zero"))
  ...
  return(list(b_pred = ga_b, rmse = rmse, betas_est = fit_PH$coefficients,
...
GetB_CoxPHmrl <- function(df_train, df_test, M, lt){
  ...
  return(list(b_pred = ph_b, mrl_pred = pred_mrl, rmse = rmse, betas_est = betas_est_ph))
...
GetB_AFTmrl_Weibull <- function(df_train, df_test, M, lt){
  ...
  return(list(b_pred = aft_b, mrl_pred = pred_mrl, rmse = rmse, betas_est = betas_est,
...
GetB_AFTmrl_semipar <- function(df_train, df_test, M, lt){
  ...
  return(list(b_pred = aft_b, mrl_pred = pred_mrl, rmse = rmse, betas_est = lm.1$coefficients))
  
}

################################################################
# Method F: flexible parametric AFT

GetB_AFTmrl_flexpar <- function(df_train, df_test, M, lt, df_splines){
  
  
  # fit regression model 
  fit_aft <- rstpm2::aft(formula = Surv(c, age_end, status) ~ V1 + V2, 
                         data = df_train, df = df_splines)
  
  # obtain baseline survival 
  base_surv <- predict(fit_aft, newdata=data.frame(V1 = 0, V2 = 0), grid = T, type = "surv", seqLength = 300)
  # grid means equally spaced event times (https://cran.r-project.org/web/packages/rstpm2/rstpm2.pdf)
  times <- seq(from = min(df_train$age_end[df_train$status == 1]), to = max(df_train$age_end[df_train$status == 1]), length.out = 299)
  
  approx_base_surv <- approxfun(times, base_surv, yleft = 1, yright = 0, n = 100)
  inverse_approx_base_surv <- approxfun(base_surv, times, yleft = max(times), yright = min(times), n = 100)
  
  # obtain linear predictors for test data
  betas_est <- coef(fit_aft)[1:M] * -1
  linpred_aft <- rowSums(sweep(df_test[,1:M], 2, betas_est, "*"))
