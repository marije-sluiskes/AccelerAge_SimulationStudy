...
save(params, file = "output/scenarios/Gompertz_PH/simulated/params")
...
for (i in 1:N_pop){
  t[i] <- rgompertz_ph(1, a = a, b = b, linpred = sum(betas * X[i,])) 
}
...
save(lt, file = "output/scenarios/Gompertz_PH/simulated/lifetable")
...
for (i in 1:nrow(df_sim)){                                                    
  surv_prob <- gomp_baseline_surv(df_sim$c[i], a = a, b = b)^(exp(df_sim$linpred[i]))
  med_prob_adj <- (surv_prob / 2)^(1/exp(df_sim$linpred[i]))
  t_adj <- inverse_gomp_baseline_surv(med_prob_adj, a=a, b=b)
...
df_test <- df_sim
save(df_test, file = "output/scenarios/Gompertz_PH/simulated/df_test")
...
for (k in 1:length(n_obs)){ # looping over n_obs (number of observations within each dataset)
  ...
  save(list_df_iter, file = paste0("output/scenarios/Gompertz_PH/simulated/list_df_iter-n_obs", n_obs[k]))
